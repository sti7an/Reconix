...
censys_subdomains() {
    ...
    if [[ -f "$script" ]]; then
        ...
        recon_log_error "censys-subdomain-finder not found in ${RECONIX_TOOLS_DIR}"
        return 1
    fi
}
...
remote_eyewitness() {
    ...
    ssh -n -f "${key_opt[@]}" -t "$ssh_target" \
        "bash -ic 'cd ${VPS_WORK_DIR}/${corp}/ && nohup python3 ${RECONIX_TOOLS_DIR}/EyeWitness/Python/EyeWitness.py --no-prompt --timeout 20 --delay 10 --max-retries 5 -f alive.txt -d dump-screenshots > /dev/null 2>&1 &'"
}
