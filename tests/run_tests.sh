...
# Unit tests for Reconix core functions
...
if grep -qE 'AAE|1546805674|4bc14ef862090a3426cbb6157ecf391289cdbc6d|23\.227\.206\.164' "${RECONIX_ROOT}/config/defaults.env" 2>/dev/null; then
...
if grep -rqE '1546805674|4bc14ef862090a3426cbb6157ecf391289cdbc6d|23\.227\.206\.164|AAEVPN-USER02|Aae@123|32038511e1ca2c|28d4930e-dd8d' \
    "${RECONIX_ROOT}/core" "${RECONIX_ROOT}/commands" "${RECONIX_ROOT}/plugins" "${RECONIX_ROOT}/workflows" "${RECONIX_ROOT}/reconix" 2>/dev/null; then
...
# Branding
if [[ -x "${RECONIX_ROOT}/reconix" ]]; then
    echo "PASS: reconix executable exists"
    PASS=$((PASS + 1))
else
    echo "FAIL: reconix executable missing"
    FAIL=$((FAIL + 1))
fi
