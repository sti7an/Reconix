...
_recon_log_write() {
    ...
    if [[ -n "${RECON_LOG_FILE:-}" ]]; then
        ...
        echo "$line" >> "${RECON_LOG_DIR}/recon-tools.log"
    fi
}
...
recon_init_session_log() {
    ...
    mkdir -p "${RECON_LOG_DIR:-${RECON_TOOLS_ROOT}/logs}" 2>/dev/null || true
    RECON_LOG_FILE="${RECON_LOG_DIR:-${RECON_TOOLS_ROOT}/logs}/${name}-$(date '+%Y%m%d-%H%M%S').log"
